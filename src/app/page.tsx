...
import { Star, Crown, Coins, Settings, ShoppingBag, User, Search, ChevronRight } from "lucide-react";
...
const Section = ({ title, children, cta }: { title: string; children: React.ReactNode; cta?: React.ReactNode }) => (
  <section className="mt-8">
    <div className="flex items-center justify-between mb-3">
      <h2 className="text-lg font-semibold tracking-wide text-amber-200 drop-shadow-[0_1px_0_rgba(0,0,0,.8)]">{title}</h2>
      {cta}
    </div>
    {children}
  </section>
    ...
    >
      ...
      <div className="absolute bottom-0 left-0 right-0 p-3 bg-gradient-to-t from-black/70 to-transparent">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold tracking-wide text-amber-200">{name}</span>
        </div>
      </div>
    </motion.button>
  );
    ...
    <div className="flex items-center gap-3 px-4 py-2 bg-zinc-950/70 border-b border-amber-400/10 backdrop-blur supports-[backdrop-filter]:bg-zinc-950/50">
      ...
      <div className="ml-auto flex items-center gap-2">
        <div className="hidden md:flex items-center gap-2 px-3 py-1.5 rounded-full bg-zinc-800/60 border border-zinc-700">
          ...
          <input placeholder="Search games" className="bg-transparent outline-none text-sm placeholder:text-zinc-500" />
        </div>
        <button className="inline-flex items-center gap-2 rounded-full bg-amber-500 text-zinc-950 px-4 py-1.5 text-sm font-semibold shadow hover:bg-amber-400"><ShoppingBag className="size-4"/> Store</button>
        <button className="inline-flex items-center gap-2 rounded-full bg-zinc-800 border border-zinc-700 px-4 py-1.5 text-sm text-zinc-200 hover:bg-zinc-700"><User className="size-4"/> Account</button>
        <button className="hidden sm:inline-flex items-center gap-2 rounded-full bg-zinc-800 border border-zinc-700 px-3 py-1.5 text-sm text-zinc-200 hover:bg-zinc-700"><Settings className="size-4"/> Settings</button>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="relative flex flex-col items-center text-center py-6">
      ...
      <div className="relative">
        <motion.div initial={{ scale: .95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ duration: .6, ease: 'easeOut' }} className="px-8 py-4 rounded-[24px] border-2 border-amber-400/70 bg-gradient-to-b from-zinc-900 to-zinc-950 shadow-[0_0_0_6px_rgba(217,119,6,.15),0_12px_40px_rgba(0,0,0,.6)]">
          <div className="text-[clamp(24px,5vw,44px)] font-extrabold tracking-wider text-amber-300 drop-shadow">SILVER COIN</div>
          <div className="mt-1 text-[clamp(10px,2.6vw,14px)] tracking-[0.35em] text-amber-200/80">SOCIAL CASINO</div>
        </motion.div>
      </div>
      <p className="relative mt-3 text-sm text-amber-100/70 max-w-prose">A nostalgic Rat Pack–era vibe with modern polish. Play slots, table games, and cards in Gold or Sweeps mode.</p>
    </div>
  );
}
  ...
  return (
    <div className="mt-10 mb-8">
      <button className="group inline-flex items-center gap-2 rounded-full bg-amber-500 text-zinc-950 px-6 py-2 font-semibold shadow hover:bg-amber-400">
        Browse All Games <ChevronRight className="size-4 transition-transform group-hover:translate-x-0.5" />
      </button>
    </div>
  );
}
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#1a0f08] via-[#2a1309] to-[#120904] text-amber-50">
      ...
      <main className="mx-auto max-w-7xl px-4 sm:px-6">
        ...
        <Section title="Slots" cta={<a className="text-xs text-amber-300 hover:underline" href="#">See all</a>}>
          <Grid items={SLOTS} />
        </Section>
